...
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        ...
        if(ep == 0 and batch == 0):
            hvd.broadcast_variables(mnist_model.trainable_variables, root_rank=0)
        ...
        #print("train:",hvd.rank(),training_loss.numpy(),total_loss.numpy(),training_acc.numpy(),total_acc.numpy())
    ...
    for batch, (images, labels) in enumerate(test_dset.take(ntest_step)):
        ...
        total_acc = hvd.allreduce(test_acc, average=True)
        total_loss = hvd.allreduce(test_loss, average=True)
        #print("test:",hvd.rank(),test_loss.numpy(),total_loss.numpy(),test_acc.numpy(),total_acc.numpy())
        test_acc = total_acc
        test_loss = total_loss
    tt1 = time.time()
    if(hvd.rank() == 0): print('%d,%.3f,%.3f' % (ep, training_acc, test_acc))
